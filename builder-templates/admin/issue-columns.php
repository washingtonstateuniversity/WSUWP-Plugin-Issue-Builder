...
<div class="wsuwp-issue-stage">
	...
	foreach ( $section_order as $key => $i ) :
		...
		$issue_post_id = ( isset( $ttfmake_section_data['data']['columns'][ $i ]['post-id'] ) ) ? $ttfmake_section_data['data']['columns'][ $i ]['post-id'] : '';
		...
		<div class="wsuwp-spine-builder-column wsuwp-spine-builder-column-position-<?php echo esc_attr( $j ); ?>" data-id="<?php echo esc_attr( $i ); ?>">
			...
			<input type="hidden" class="wsuwp-column-post-id wsuwp-issue-post-meta" name="<?php echo esc_attr( $column_name ); ?>[post-id]" value="<?php echo esc_attr( $issue_post_id ); ?>" />
			...
			if ( $issue_post_id ) :
				...
				$header = ( isset( $ttfmake_section_data['data']['columns'][ $i ]['header'] ) ) ? $ttfmake_section_data['data']['columns'][ $i ]['header'] : get_the_title( $issue_post_id );
				...
				?>
				<div id="issue-post-<?php echo esc_attr( $issue_post_id ); ?>"
					class="issue-post"
					...
					<div class="ttfmake-sortable-handle ui-sortable-handle" title="Drag and drop this post into place">
						...
						<div class="wsuwp-builder-column-title"><?php echo get_the_title( $issue_post_id ); ?></div>
					...
					<article class="<?php echo esc_attr( $post_classes ); ?>" style="<?php echo esc_attr( $post_styles ); ?>">
						...
						<figure class="featured-image">
							<img src="<?php echo esc_url( get_the_post_thumbnail_url( $issue_post_id, 'spine-large_size' ) ); ?>" />
						</figure>

						<figure class="thumbnail-image">
							<img src="<?php echo esc_url( MultiPostThumbnails::get_post_thumbnail_url( 'post', 'thumbnail-image', $issue_post_id, 'spine-large_size' ) ); ?>" />
						</figure>

						<div class="excerpt">
							<?php if ( has_excerpt( $issue_post_id ) ) { echo wp_kses_post( wpautop( get_the_excerpt( $issue_post_id ) ) ); } ?>
						</div>
